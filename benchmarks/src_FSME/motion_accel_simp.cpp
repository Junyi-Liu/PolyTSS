											...
											for(int ty=0; ty<16; ty++){
												for(int tx=0; tx<16; tx++){
													...
													for(int tsx=sx; tsx<sx+dsx; tsx++){
														...
														acc[tsx-sx] = tmp + abs(buf_in[tc][tby*16+ty][tbx*16+tx] - buf_ref[tr][tby*16+tsy+ty][tbx*16+tsx-sx+tx]);

														if((y+ty==15) && (x+tx==15)){
															if(acc[tsx-sx]<minimum[tc][tby][tbx]){//check if current score is better than best match until now
																minimum[tc][tby][tbx]=acc[tsx-sx];
																vec[tc][tby][tbx]=((ref+tr)<<15)|(((sy+tsy)<<8)|tsx);//encode reference (bit 15), encode y direction (bit 11:8), encode x direction (bit 3:0)
//																vec[tc][tby][tbx]=acc[tsx-sx];
															}
														}
