							...
							read_fin:for (int tn=n; tn<min(n+Tn,N); tn++){
								for (int tr=ST*row+i; tr<min(ST*(row+Tr-1)+i+Ti,inRow); tr++){
									for (int tc=ST*col+j; tc<min(ST*(col+Tc-1)+j+Tj,inCol); tc++){
										#pragma HLS PIPELINE
										buf_fin[tn-n][tr-ST*row-i][tc-ST*col-j] = *(frame_in + tn*inRow*inCol+tr*inCol+tc);
									}
								}
							}
...
//							std::cout << "!!! Tile begin" << std::endl;
							// basic pipeline
//							tile_comp:for (int tm=m; tm<min(m+Tm,M); tm++) {
...
//												for(int tj=j; tj<min(j+Tj,K); tj++) {
//													#pragma HLS PIPELINE
//													buf_fout[tm-m][tr-row][tc-col] += buf_wgt[tm-m][tn-n][ti-i][tj-j] * buf_fin[tn-n][ST*(tr-row)+ti-i][ST*(tc-col)+tj-j];
//												}
//											}
//										}
//									}
											...
											#pragma HLS unroll factor=16
												...
												for (int tn=0; tn<N; tn++) {
													 if(tr+row<outRow && tc+col<outCol && ti+i<K && tj+j<K && tm+m<M && tn+n<N)
														 tmp += buf_wgt[tm][tn][ti][tj] * buf_fin[tn][ST*tr+ti][ST*tc+tj];
												}
