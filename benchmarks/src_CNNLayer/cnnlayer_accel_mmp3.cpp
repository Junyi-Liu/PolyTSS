...
int cnnlayer_accel_mmp3(data_t *frame_in, data_t *weight, data_t *frame_out)
{
	...
	data_t buf_fin[Tn][Tr][ST*(Tc-1)+Tj]; // Ti = 1
							...
							read_fin:for (int tn=n; tn<min(n+Tn,N); tn++){
								for (int tr=row; tr<min(row+Tr,outRow); tr++) {
									for (int tc=ST*col+j; tc<min(ST*(col+Tc-1)+j+Tj,inCol); tc++){
										#pragma HLS PIPELINE
										buf_fin[tn-n][tr-row][tc-ST*col-j] = *(frame_in + tn*inRow*inCol+(ST*tr+i)*inCol+tc);
									}
								}
							}
										...
										#pragma HLS PIPELINE
											...
											#pragma HLS unroll factor=16
												...
												for (int tn=0; tn<N; tn++) {
													 if(tr+row<outRow && tc+col<outCol && tj+j<K && tm+m<M && tn+n<N)
														 tmp += buf_wgt[tm][tn][tj] * buf_fin[tn][tr][ST*tc+tj];
												}
												if(tr+row<outRow && tc+col<outCol && tj+j<K && tm+m<M)
