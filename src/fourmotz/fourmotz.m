...
if n > nvar,
    ...
    seq = 1:m;
	temp = [A,seq'];			% compound matrix [A|b]
	temp = sortrows(temp,1);	% sort according to first column
    b = b(int32(temp(:,n+1)));
    temp = [temp(:,1:n),b];
    
	nneg = sum(double(temp(:,1))<0);	% count negative entries in first column
	nzer = sum(double(temp(:,1))==0);	% count zero entries in first column
	npos = sum(double(temp(:,1))>0);	% count positive entries in first column
	...
	A = double(A(:,1:n-1));			% reduce to the relevant A
